...
0. ${Green_font_prefix} 一定一定先screen ${Font_color_suffix}
...
6. 不支持centos" && echo stty erase '^H' && read -p "(默认: 取消):" need
...
echo -e "选择安装内容
1. lnmp
2. lamp" && echo stty erase '^H' && read -p "(默认: 取消):" web
[[ -z "${web}" ]] && echo "已取消..." && exit 1
if [[ ${web} == "1"]]; then
	wget -c http://soft.vpser.net/lnmp/lnmp1.5.tar.gz && tar zxf lnmp1.5.tar.gz && cd lnmp1.5 && ./install.sh lnmp
	elif [[ ${web} == "2"]]; then
	apt-get -y install wget screen git && git clone https://github.com/teddysun/lamp.git && cd lamp && chmod +x *.sh && ./lamp.sh
elif [[ ${need} == "5" ]]; then
